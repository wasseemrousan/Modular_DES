function  [G,Nodes_Number,Nodes_Names] = ConvertAutomaton_new_char(G1)
...
%              names = states;
    ...
    sourcetstates(w) = strtrim(sourcetstates(w));
    ...
    for z= 2:size(space_id__events,2)
            ...
            if (((space_id__events(z)) - space_id__events(z-1)) ~= 1) && (j==2)
                ...
                endstates(w) = strtrim(endstates(w));
...
%     G = digraph(sourcetstatesdouble,endstatesdouble,ones(size(EdgesName,2),1));
    ...
    EdgeTable = table([sourcetstates' endstates'],ones(size(EdgesName,2),1),EdgesName', ...
