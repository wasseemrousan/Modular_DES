function [ps_agents, ps, MW_lost, imbalance] = distributed_control(ps_agents,ps,sub_grids,ramp_rate,it_no,opt,pf_model,currentstates)
% distributed emergency control
C = psconstants;
EPS = 1e-4; % wasseem . it was 1e-4 and i changed it to 0 to avoid a recurrent error in P
nbus = size(ps.bus,1);
...
if abs(MW_lost_total - (Pd0_sum - Pd_sum)) > EPS
    ...
    function approved = check_capacity(delta_Pd,delta_Pg,bus_i_f,local_nei,verbose)
        ...
        for j = local_nei
            ...
            if all(Pg_after >= 0) % Wasseem ; changed from EPS to >= 0 
                gen_approved = true;
            else
                gen_approved = false;
            end
