function [is_blackout,relay_outages,MW_lost,p_out,busessep,movie_data,n_msg] = dcsimsep(ps,br_outages,bus_outages,opt,currentstates,outputtreeObject,Npointer,pre_contingency_flows)  
...
% Index = 0; % this is the case when we don't use control, just to make the function work 
% check the inputs
if nargin<3
    bus_outages = [];
end
...
if all(ramp_rate==0)
    ramp_rate_MW_per_min = max(1,Pg_max*.05); % assume that all plants can ramp at 30% per minute (Wasseem changed to 30%, it was 5%). 
...
%         if opt.debug && any( Pg<Pg_min | Pg>Pg_max ), error('Pg is out of bounds'); end
    ...
    % wasseem, the control actions maybe implemented here 
    ...
    if ~strcmp(opt.sim.control_method, 'none') && ...
        ...
        switch opt.sim.control_method
            ...
            case 'distributed_control' % distributed emergency control 
                [ps_agents, ps, this_MW_lost, mis,is_blackout] = distributed_control(ps_agents,ps,sub_grids,ramp_rate,it_no,opt,'dc',currentstates);
%+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++%                
            case 'DEScontrol' % Wasseem 11/24/2019, use this part to start calling the LLP algoritm
                disp ('well use DES CENTRAL CONTROL')
...
%+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++%
    ...
    %% Wasseem 06/17/2021 Maybe update the pointer here after rely outages being recorded 
